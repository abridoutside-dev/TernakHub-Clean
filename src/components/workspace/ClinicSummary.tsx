// ─── ClinicSummary Component (CLN-002B) ───────────────────────────────────────
// Displays the Statistik Klinik summary cards for a Klinik Hewan Workspace.
// All data is received through props — no data imports inside this component.

import { type ClinicWorkspaceSummary } from '../../data/clinicWorkspaceData';
...
interface ClinicSummaryProps {
  summary: ClinicWorkspaceSummary;
}

export default function ClinicSummary({ summary }: ClinicSummaryProps) {
  return (
    <div style={{
      background: 'var(--color-surface)',
      border: '1px solid var(--color-border)',
      ...
      <SectionHeader title="Statistik Klinik" />
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <StatCard icon="👨‍⚕️" value={summary.totalDokter}           label="Dokter Hewan"    sub={`${summary.dokterAktif} aktif`} />
        <StatCard icon="👥"   value={summary.totalStaf}             label="Total Staf" />
        <StatCard icon="🛏️"  value={summary.kapasitasRawatInap}    label="Kapasitas Rawat" sub="inap (ekor)" />
        <StatCard icon="🐄"   value={summary.pasienAktif}           label="Pasien Aktif" />
        <StatCard icon="📋"   value={summary.kunjunganBulanIni}     label="Kunjungan"       sub="bulan ini" />
