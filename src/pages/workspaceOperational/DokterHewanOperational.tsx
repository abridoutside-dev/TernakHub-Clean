// ─── DokterHewanOperational — VET-OPS-001 ─────────────────────────────────────
// Dashboard Operasional Workspace Dokter Hewan.
...
//   layanan_dokter_hewan     → Katalog Layanan
//   activity_log             → Aktivitas workspace
//
// Fitur:
//   - Ringkasan data live
//   - Daftar layanan dengan CRUD
//   - Daftar pemeriksaan dengan create
...
import {
  repoGetVetServicesByWorkspace,
  repoInsertVetService,
  repoPatchVetService,
  repoDeleteVetService,
...
function ServiceFormModal({ onClose, onSave, initial }: {
  onClose: () => void; onSave: (data: VetServiceCreateInput) => void; initial?: VetServiceDbRow | null;
}) {
  const [nama, setNama] = useState(initial?.nama ?? '');
  const [kategori, setKategori] = useState(initial?.kategori ?? '');
  const [spesialisasi, setSpesialisasi] = useState(initial?.spesialisasi.join(', ') ?? '');
  const [hewan, setHewan] = useState(initial?.hewan_ditangani.join(', ') ?? '');
  const [mode, setMode] = useState(initial?.mode_pelayanan.join(', ') ?? '');
  ...
  function handleSubmit() {
    ...
    try {
      onSave({
        ...
        nama: nama.trim(),
        nama_klinik: initial?.nama_klinik ?? null,
        kategori: kategori.trim() || null,
        sub_kategori: initial?.sub_kategori ?? null,
        sipv_number: initial?.sipv_number ?? null,
        spesialisasi: spesialisasi.split(',').map(s => s.trim()).filter(Boolean),
        hewan_ditangani: hewan.split(',').map(s => s.trim()).filter(Boolean),
        mode_pelayanan: mode.split(',').map(s => s.trim()).filter(Boolean),
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <p style={{ margin: '0 0 16px', fontSize: 18, fontWeight: 800 }}>{initial ? 'Edit Layanan' : 'Layanan Baru'}</p>
      {error && <p style={{ color: '#dc2626', fontSize: 13, marginBottom: 12 }}>{error}</p>}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <label style={labelStyle}>Nama Layanan *<input style={inputStyle} value={nama} onChange={e => setNama(e.target.value)} /></label>
        <label style={labelStyle}>Kategori<input style={inputStyle} value={kategori} onChange={e => setKategori(e.target.value)} /></label>
        <label style={labelStyle}>Spesialisasi (pisah koma)<input style={inputStyle} value={spesialisasi} onChange={e => setSpesialisasi(e.target.value)} /></label>
        ...
        <label style={labelStyle}>Mode Pelayanan (pisah koma)<input style={inputStyle} value={mode} onChange={e => setMode(e.target.value)} /></label>
...
function TreatmentFormModal({ onClose, onSave, checkupId }: {
  onClose: () => void; onSave: (data: HealthTreatmentCreateInput) => void; checkupId?: string;
}) {
  const [date, setDate] = useState(new Date().toISOString().slice(0, 10));
...
export default function DokterHewanOperational(): React.ReactElement {
  ...
  const config = getWorkspaceOperationalConfig('DokterHewan');
  ...
  const [services, setServices] = useState<VetServiceDbRow[]>([]);
  ...
  const loadServices = useCallback(async () => {
    ...
    setServicesError('');
    try {
      const rows = await repoGetVetServicesByWorkspace(workspaceId);
      setServices(rows);
    } catch (e) {
  ...
  // ── Modal states ───────────────────────────────────────────────────────────
  const [serviceModal, setServiceModal] = useState<{ mode: 'create' | 'edit'; initial?: VetServiceDbRow } | null>(null);
  ...
  // ── Handlers ───────────────────────────────────────────────────────────────
  async function handleSaveService(input: VetServiceCreateInput) {
    if (serviceModal?.mode === 'edit' && serviceModal.initial) {
      await repoPatchVetService(serviceModal.initial.id, input);
    } else {
      await repoInsertVetService({ ...input, workspace_id: workspaceId });
    }
      ...
      {dashboardConfig.quickActions.length > 0 && (
        <section style={{ background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: 'var(--radius-md)', padding: 14, marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 10 }}>
            <div>
              <p style={{ margin: 0, fontSize: 12, fontWeight: 800, color: '#6d1230' }}>Quick Action</p>
              <p style={{ margin: '3px 0 0', fontSize: 11, color: COLORS.text }}>Akses cepat praktik dokter</p>
            </div>
        ...
        <WorkspaceCard style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 8 }}>
            {[
              ...
              { label: 'Pemeriksaan', value: formatNumber(checkupCount), icon: '🩺', color: '#1d4ed8', bg: '#eff6ff' },
      ...
      {!loading && (
        <>
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 800 }}>📋 Katalog Layanan</p>
              <button onClick={() => setServiceModal({ mode: 'create' })} style={{ ...btnStyle, background: COLORS.primary, color: '#fff' }}>+ Layanan</button>
            </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10 }}>
                {services.map(s => (
                  <div key={s.id} style={{ background: 'var(--color-surface)', border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: 12 }}>
                    <p style={{ margin: '0 0 4px', fontSize: 13, fontWeight: 700 }}>{s.nama}</p>
                    <p style={{ margin: '0 0 4px', fontSize: 11, color: 'var(--color-muted)' }}>{s.kategori ?? '-'} · {s.status}</p>
                    <p style={{ margin: '0 0 8px', fontSize: 10, color: 'var(--color-muted)' }}>{s.hewan_ditangani.join(', ') || '-'} · {s.mode_pelayanan.join(', ') || '-'}</p>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button onClick={() => { if (confirm('Hapus layanan ini?')) handleDeleteService(s.id); }} style={{ ...btnStyle, background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', fontSize: 11 }}>Hapus</button>
                    </div>
                  </div>
                ))}
                {services.length === 0 && <p style={{ fontSize: 12, color: 'var(--color-muted)' }}>Belum ada layanan. Klik "+ Layanan" untuk menambahkan.</p>}
              </div>
            )}
          </div>
          ...
          <DetailTable
            ...
            onRefresh={refresh}
          />
          <button onClick={() => setCheckupModal(true)} style={{ ...btnStyle, background: COLORS.primary, color: '#fff', marginBottom: 16 }}>+ Pemeriksaan Baru</button>
