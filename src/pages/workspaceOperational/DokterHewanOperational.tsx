// ─── DokterHewanOperational — ADMIN-SYNC-007 ─────────────────────────────────
// Dashboard Operasional khusus Workspace Dokter Hewan.
...
//   - Katalog Layanan      → tidak ada tabel vet_services di Supabase
//   - Area Layanan         → tidak ada tabel vet_service_areas di Supabase
...
export default function DokterHewanOperational(): React.ReactElement {
  ...
  interface OperationalSection {
    ...
    blocker?:    { reason: string; dependency: string; priority: 'high' | 'medium' | 'low' };
  }
  ...
  const OPERATIONAL_SECTIONS: OperationalSection[] = [
    {
      ...
      icon:        '🐄',
      title:       'Pasien',
      description: 'Unik livestock yang pernah diperiksa (health_checkups.livestock_id).',
    ...
    },
    {
      ...
      icon:        '🩺',
      title:       'Pemeriksaan',
      description: 'Riwayat seluruh pemeriksaan dari health_checkups.',
    ...
    },
    {
      ...
      icon:        '📋',
      title:       'Kunjungan',
      description: 'Setiap pemeriksaan = satu kunjungan (health_checkups).',
      count:       `${formatNumber(checkupCount)} kunjungan tercatat`,
      status:      'live',
    },
    {
      id:          'diagnosis',
      icon:        '🔬',
      title:       'Diagnosis',
      description: 'Pemeriksaan yang sudah memiliki diagnosis (health_checkups.diagnosis).',
    ...
    },
    {
      ...
      icon:        '💊',
      title:       'Resep & Obat',
      description: 'Tindakan yang melibatkan pemberian obat (health_treatments.drug_name).',
    ...
    },
    {
      ...
      icon:        '📅',
      title:       'Jadwal',
      description: 'Jadwal kontrol dari health_control_schedules.',
    ...
    },
    {
      ...
      icon:        '💰',
      title:       'Transaksi',
      description: 'Tindakan dengan biaya (health_treatments.cost > 0).',
  ...
  // ── Blocked Modules (dependency platform belum tersedia) ──────────────────
  const BLOCKED_MODULES: Array<{ id: string; icon: string; title: string; reason: string; dependency: string; priority: 'high' | 'medium' | 'low' }> = [
    {
      id:         'daftar-dokter',
      icon:       '👨‍⚕️',
      title:      'Daftar Dokter/Staff',
      reason:     'Tidak ada tabel khusus vet_staff di Supabase. Data dokter saat ini disimpan di data layer statis.',
      dependency: 'Tabel: vet_staff (workspace_id, nama, gelar, spesialisasi, nomorSIPP, status)',
      priority:   'high',
    },
    {
      id:         'katalog-layanan',
      ...
      reason:     'Tidak ada tabel vet_services di Supabase. Layanan saat ini disimpan di data layer statis.',
      dependency: 'Tabel: vet_services (workspace_id, tipe_layanan, harga, estimasi_durasi)',
      priority:   'medium',
    },
    {
      id:         'area-layanan',
      icon:       '🗺️',
      title:      'Area Layanan',
      reason:     'Tidak ada tabel vet_service_areas di Supabase. Area layanan disimpan di data layer statis.',
      dependency: 'Tabel: vet_service_areas (workspace_id, nama_wilayah, provinsi, jarak_maks)',
      priority:   'low',
    },
  ];
