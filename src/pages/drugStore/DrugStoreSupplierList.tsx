...
export default function DrugStoreSupplierList() {
  ...
  const load = useCallback(async () => {
    ...
    try { setSuppliers(await repoGetDrugStoreSuppliersByWorkspace(workspaceId)); }
  ...
  async function handleDelete(id: string) {
    setDeletingId(id);
    try {
      await repoDeleteDrugStoreSupplier(id);
  ...
  return (
    <main style={{ maxWidth: 760, margin: '0 auto', padding: '18px 16px 32px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
        ...
        <button type="button" onClick={() => navigate(`/workspace/${workspaceId}/drug-store/suppliers/new`)}
      ...
      {!loading && suppliers.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          <p style={{ fontSize: 12, color: '#6b7280' }}>Tambahkan pemasok obat pertama Anda.</p>
        </div>
      )}
          ...
          <div key={sup.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                <button type="button"
                  onClick={() => navigate(`/workspace/${workspaceId}/drug-store/suppliers/${sup.id}`)}
                ...
                <button type="button"
                  onClick={() => navigate(`/workspace/${workspaceId}/drug-store/suppliers/${sup.id}/edit`)}
                  style={{ border: '1px solid #d1d5db', background: '#f9fafb', borderRadius: 8, padding: '6px 12px', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>
                  Edit
                </button>
                {sup.status === 'Nonaktif' && (
                  <button type="button"
                    ...
                    style={{
                      ...
                      opacity: deletingId === sup.id ? 0.5 : 1,
                    }}>
                    {confirmId === sup.id ? '↕ Hapus?' : 'Hapus'}
                  </button>
                )}
              </div>
            </div>
