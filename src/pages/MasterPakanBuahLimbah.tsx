...
import {
  getBuahLimbahList, computeBuahLimbahRingkasan, BUAH_LIMBAH_BUAH_KATEGORI_ORDER,
  KATEGORI_ITEM_STYLE,
  type BuahLimbahBuahItem,
} from '../data/buahLimbahBuahData';
...
function computeBuahLimbahInsights(): Insight[] {
  const items   = getBuahLimbahList();
  const priced  = items.filter(i => i.estimasiHarga !== null);
  const termurah = [...priced].sort((a, b) => a.estimasiHarga! - b.estimasiHarga!)[0];
  const buahSegar = items.filter(i => i.kategoriItem === 'Buah Segar').length;
  const kulit    = items.filter(i => i.kategoriItem === 'Kulit Buah').length;
  const ampas    = items.filter(i => i.kategoriItem === 'Ampas Buah').length;
  const biji     = items.filter(i => i.kategoriItem === 'Biji Buah').length;

  return [
    {
      icon: '🍌', color: '#f57f17', bg: '#fff9c4',
      text: `${buahSegar} jenis buah segar terdokumentasi — buah afkir dan sisa sortasi pasar adalah sumber karbohidrat dan palatabilitas ekonomis yang sering diabaikan peternak. Pisang, pepaya, dan nangka afkir bisa diperoleh hampir gratis dari pedagang pasar.`,
    },
    {
      icon: '🍊', color: '#e65100', bg: '#fff3e0',
      text: `Ampas jeruk (Dried Citrus Pulp/DCP) adalah bahan pakan internasional dengan TDN ±75% — setara sumber energi yang baik. Pektin tinggi (±24% BK) memperbaiki fermentasi rumen. Dapat menggantikan 15–20% jagung dalam konsentrat ruminansia.`,
    },
    {
      icon: '💰', color: '#1b7a43', bg: '#e8f5ee',
      text: `${termurah ? `${termurah.nama} (Rp ${termurah.estimasiHarga!.toLocaleString('id-ID')}/kg)` : 'Kulit buah'} adalah bahan termurah dalam kategori ini. ${kulit} jenis kulit buah dan ${ampas} jenis ampas buah tersedia sebagai limbah zero-cost atau biaya minimal dari pasar dan pabrik pengolahan.`,
    },
    {
      icon: '⚠️', color: '#c0392b', bg: '#fdecea',
      text: `Perhatikan antinutrisi pada biji buah: theobromin pada biji kakao (berbahaya untuk kuda dan babi), kafein pada biji kopi (batasi ≤2% ransum), karpain pada biji pepaya. Ruminansia lebih toleran, tetapi tetap perlu adaptasi bertahap.`,
    },
    {
      icon: '🧪', color: '#6d4c41', bg: '#efebe9',
      text: `${biji} jenis biji buah dalam kategori ini memiliki profil protein dan lemak tinggi — biji semangka (protein ±28%, lemak ±47% BK) dan biji pepaya (protein ±24–30% BK) berpotensi sebagai pengganti parsial bungkil kedelai jika tersedia dalam volume cukup.`,
    },
    {
      icon: '🌿', color: '#2e7d32', bg: '#e8f5e9',
      text: `Kulit kakao (pod kakao) adalah sumber serat lokal paling berlimpah di sentra perkebunan kakao (Sulawesi, Papua). Protein ±8% BK dengan theobromin rendah di kulit pod — aman untuk sapi dan kambing. Silase kulit kakao meningkatkan kecernaan dan umur simpan secara signifikan.`,
    },
  ];
}

function BuahLimbahAiInsightCard() {
  const [expanded, setExpanded] = useState(false);
  const insights = computeBuahLimbahInsights();
  ...
  return (
    <div style={{
      ...
      <div style={{ background: 'var(--color-primary)', padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 800, color: '#fff', flex: 1 }}>AI Insight — Buah & Limbah Buah</span>
...
function BuahLimbahRingkasan() {
  const r = computeBuahLimbahRingkasan();

  const cards = [
    {
      label: 'Total Referensi Buah & Limbah',
...
function BuahLimbahItemCard({ item, onDetail }: { item: BuahLimbahBuahItem; onDetail: () => void }) {
...
export default function MasterPakanBuahLimbah() {
  const navigate = useNavigate();
  const [query, setQuery] = useState('');

  const allItems = getBuahLimbahList();
  ...
  const grouped = BUAH_LIMBAH_BUAH_KATEGORI_ORDER
    .map(kat => ({
      kat,
    ...
    <div style={{ paddingBottom: 80 }}>
      <div style={{ padding: '16px 16px 0', maxWidth: 480, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{
            ...
            fontSize: 28, border: '1.5px solid #f57f1744',
          }}>
            🍌
          </div>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--color-muted)', marginTop: 2 }}>
              {allItems.length} referensi bahan baku buah & limbah buah
            </div>
          </div>
        </div>

        {/* AI Insight */}
        <BuahLimbahAiInsightCard />
      ...
      <div style={{ padding: '14px 16px 0', maxWidth: 480, margin: '0 auto' }}>
        <div style={{
          ...
          <input
            type="text"
            placeholder="Cari nama buah atau limbah buah..."
            value={query}
            ...
            <div key={kat} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {items.map(item => (
                <BuahLimbahItemCard
                  key={item.id}
                  item={item}
                  onDetail={() => navigate(`/stok-pakan/master/buah-limbah-buah/${item.id}`)}
                />
              ))}
            </div>
          ))
