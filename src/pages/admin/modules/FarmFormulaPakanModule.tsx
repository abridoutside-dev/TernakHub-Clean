// ─── Farm Formula Pakan — ADMIN-SYNC-004 ─────────────────────────────────────
// Cross-workspace admin view of the `feed_formulas` table.
...
interface FormulaRow {
  ...
  workspaces?: { name?: string | null; plan?: string | null; owner_name?: string | null } | null;
}
...
export default function FarmFormulaPakanModule() {
    ...
    (async () => {
      try {
        ...
        const [countRes, dataRes] = await Promise.all([
          ...
          supabase.from('feed_formulas')
            .select('*, workspaces(name, plan, owner_name)')
        ...
        setRows((dataRes.data ?? []) as FormulaRow[]);
      } catch (e: unknown) {
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: '#3b82f6', fontWeight: 600 }}>Formula Pakan</span>
          </div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: '#0f172a', letterSpacing: -0.3 }}>🧪 Formula Pakan</h1>
          <p style={{ margin: '6px 0 0', fontSize: 13.5, color: '#64748b' }}>
            Formula pakan seluruh platform — data dari Supabase{' '}
            <code style={{ fontSize: 12, background: '#f1f5f9', padding: '1px 5px', borderRadius: 4 }}>feed_formulas</code> table.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginBottom: 24 }}>
          {loading ? Array.from({ length: 4 }).map((_, i) => (
            ...
            {[
              { label: 'Total Formula',  value: totalCount,       icon: '🧪', color: '#3b82f6' },
              { label: 'Aktif',          value: stats.active,     icon: '✅', color: '#10b981' },
              { label: 'Draft',          value: stats.draft,      icon: '✏️', color: '#f59e0b' },
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: '16px 20px', border: '1px solid #f1f5f9', marginBottom: 20, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minWidth: 180 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: '#64748b' }}>Cari Formula</span>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Nama formula atau workspace…"
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f1f5f9', overflow: 'hidden', marginBottom: 32 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#0f172a' }}>Formula Pakan (Seluruh Platform)</span>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Nama Formula', 'Workspace', 'Status', 'Target Spesies', 'Kelompok Umur', 'Biaya/kg', 'Dibuat'].map(h => (
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} style={{ padding: '48px 20px', textAlign: 'center', color: '#94a3b8' }}>
                      ...
                      <div style={{ fontWeight: 600, color: '#64748b' }}>
                        {rows.length === 0 ? 'Belum ada formula pakan di Supabase.' : 'Tidak ada hasil'}
                      </div>
                    </td>
                  </tr>
                ) : pageRows.map((r, i) => (
                  <tr key={r.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafbfc', borderBottom: '1px solid #f1f5f9' }}>
                    <td style={{ padding: '10px 14px' }}>
                      ...
                      <div style={{ fontSize: 11, color: '#94a3b8' }}>{r.description?.substring(0, 50) ?? ''}</div>
                    </td>
                    <td style={{ padding: '10px 14px' }}>
                    ...
                    <td style={{ padding: '10px 14px', color: '#475569', fontSize: 12 }}>{(r.target_species ?? []).join(', ') || '—'}</td>
                    <td style={{ padding: '10px 14px', color: '#475569', fontSize: 12 }}>{r.target_age_group ?? '—'}</td>
                    <td style={{ padding: '10px 14px', color: '#0f172a', fontWeight: 600, fontSize: 12, whiteSpace: 'nowrap' }}>
                      {r.total_cost_per_kg != null ? `Rp ${r.total_cost_per_kg.toLocaleString('id-ID')}/kg` : '—'}
                    </td>
