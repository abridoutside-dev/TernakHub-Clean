// ─── Farm Reproduksi — ADMIN-SYNC-004 ────────────────────────────────────────
// Cross-workspace admin view of the `reproduksi_programs` table.

import { useEffect, useMemo, useState } from 'react';
import AdminLayout from '../layout/AdminLayout';
import { supabase } from '../../../lib/supabase';

interface ReproduksiRow {
  id: string;
  workspace_id: string;
  name: string;
  species: string | null;
...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  Draft:    { label: 'Draft',    color: '#2563eb', bg: '#dbeafe', dot: '#3b82f6' },
  Aktif:    { label: 'Aktif',    color: '#059669', bg: '#d1fae5', dot: '#10b981' },
  Selesai:  { label: 'Selesai',  color: '#7c3aed', bg: '#ede9fe', dot: '#8b5cf6' },
  Dibatalkan: { label: 'Dibatalkan', color: '#374151', bg: '#f3f4f6', dot: '#9ca3af' },
};
...
export default function FarmReproduksiModule() {
  const [rows, setRows]         = useState<ReproduksiRow[]>([]);
    ...
    (async () => {
      try {
        ...
        const [countRes, dataRes] = await Promise.all([
          ...
          supabase.from('reproduksi_programs')
        ...
        setRows((dataRes.data ?? []) as ReproduksiRow[]);
      } catch (e: unknown) {
  ...
  const stats = useMemo(() => ({
    ...
    workspace: new Set(rows.map(r => r.workspace_id)).size,
  }), [rows]);

  const filtered = useMemo(() => rows.filter(r => {
    if (search && !r.name.toLowerCase().includes(search.toLowerCase()) &&
        !(r.workspaces?.name ?? '').toLowerCase().includes(search.toLowerCase())) return false;
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: '#3b82f6', fontWeight: 600 }}>Reproduksi</span>
          </div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: '#0f172a', letterSpacing: -0.3 }}>🔬 Program Reproduksi</h1>
          <p style={{ margin: '6px 0 0', fontSize: 13.5, color: '#64748b' }}>
            Program reproduksi seluruh platform — data dari Supabase{' '}
            <code style={{ fontSize: 12, background: '#f1f5f9', padding: '1px 5px', borderRadius: 4 }}>reproduksi_programs</code> table.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginBottom: 24 }}>
          {loading ? Array.from({ length: 4 }).map((_, i) => (
            ...
            {[
              { label: 'Total Program',  value: totalCount,       icon: '🔬', color: '#3b82f6' },
              { label: 'Aktif',          value: stats.active,     icon: '✅', color: '#10b981' },
              { label: 'Selesai',        value: stats.done,       icon: '🏁', color: '#8b5cf6' },
              { label: 'Workspace',      value: stats.workspace,  icon: '🏢', color: '#f59e0b' },
            ].map(card => (
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: '16px 20px', border: '1px solid #f1f5f9', marginBottom: 20, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minWidth: 180 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: '#64748b' }}>Cari Program</span>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Nama program atau workspace…"
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f1f5f9', overflow: 'hidden', marginBottom: 32 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#0f172a' }}>Program Reproduksi (Seluruh Platform)</span>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Nama Program', 'Workspace', 'Spesies', 'Status', 'Metode Kawin', 'Mulai', 'Selesai', 'Dibuat'].map(h => (
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} style={{ padding: '48px 20px', textAlign: 'center', color: '#94a3b8' }}>
                      ...
                      <div style={{ fontWeight: 600, color: '#64748b' }}>
                        {rows.length === 0 ? 'Belum ada program reproduksi di Supabase.' : 'Tidak ada hasil'}
                      </div>
                    </td>
                  </tr>
                ) : pageRows.map((r, i) => (
                  <tr key={r.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafbfc', borderBottom: '1px solid #f1f5f9' }}>
                    <td style={{ padding: '10px 14px' }}>
                      ...
                      <div style={{ fontSize: 11, color: '#94a3b8' }}>{r.target_breed ?? ''}</div>
                    </td>
                    <td style={{ padding: '10px 14px' }}>
                      <div style={{ fontSize: 12.5, color: '#0f172a' }}>{r.workspaces?.name ?? '—'}</div>
                      <div style={{ fontSize: 11, color: '#94a3b8' }}>{r.workspaces?.plan ?? ''}</div>
                    </td>
                    <td style={{ padding: '10px 14px', color: '#475569', fontSize: 12 }}>{r.species ?? '—'}</td>
                    <td style={{ padding: '10px 14px' }}><StatusBadge status={r.status} /></td>
                    <td style={{ padding: '10px 14px', color: '#475569', fontSize: 12 }}>{r.mating_method ?? '—'}</td>
                    <td style={{ padding: '10px 14px', color: '#64748b', fontSize: 12, whiteSpace: 'nowrap' }}>{r.start_date ?? '—'}</td>
                    <td style={{ padding: '10px 14px', color: '#64748b', fontSize: 12, whiteSpace: 'nowrap' }}>{r.end_date ?? '—'}</td>
