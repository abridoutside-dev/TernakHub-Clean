// ─── Farm Pemberian Pakan — ADMIN-SYNC-004 ────────────────────────────────────
// Cross-workspace admin view of the `pemberian_pakan` table.

import { useEffect, useMemo, useState } from 'react';
import AdminLayout from '../layout/AdminLayout';
import { supabase } from '../../../lib/supabase';

interface PemberianPakanRow {
  id: string;
  workspace_id: string;
  feed_date: string;
...
export default function FarmPemberianPakanModule() {
  const [rows, setRows]        = useState<PemberianPakanRow[]>([]);
    ...
    (async () => {
      try {
        ...
        const [countRes, dataRes] = await Promise.all([
          ...
          supabase.from('pemberian_pakan')
            .select('*, workspaces(name, plan), livestock(name, species), batches(label)')
        ...
        setRows((dataRes.data ?? []) as PemberianPakanRow[]);
      } catch (e: unknown) {
  ...
  const stats = useMemo(() => {
    if (!rows.length) return { total: 0, totalKg: 0, avgKg: 0, workspaces: 0 };
    const totalKg = rows.reduce((s, r) => s + r.amount_kg, 0);
    return {
      total:      rows.length,
      totalKg:    Math.round(totalKg * 100) / 100,
      avgKg:      Math.round(totalKg / rows.length * 100) / 100,
      workspaces: new Set(rows.map(r => r.workspace_id)).size,
    };
  }, [rows]);
  ...
  const filtered = useMemo(() => rows.filter(r => {
    ...
    return (
      ...
      (r.batches?.label ?? '').toLowerCase().includes(q) ||
      r.feed_date.includes(q)
    );
  }), [rows, search]);
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: '#3b82f6', fontWeight: 600 }}>Pemberian Pakan</span>
          </div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: '#0f172a', letterSpacing: -0.3 }}>🌾 Pemberian Pakan</h1>
          <p style={{ margin: '6px 0 0', fontSize: 13.5, color: '#64748b' }}>
            Riwayat pemberian pakan seluruh platform — data dari Supabase{' '}
            <code style={{ fontSize: 12, background: '#f1f5f9', padding: '1px 5px', borderRadius: 4 }}>pemberian_pakan</code> table.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginBottom: 24 }}>
          {loading ? Array.from({ length: 4 }).map((_, i) => (
            ...
            {[
              { label: 'Total Pemberian', value: totalCount.toLocaleString('id-ID'),                  icon: '🌾', color: '#3b82f6' },
              { label: 'Total (kg)',       value: stats.totalKg.toLocaleString('id-ID'),              icon: '⚖️', color: '#10b981' },
              { label: 'Rata-rata (kg)',   value: stats.avgKg.toLocaleString('id-ID'),                icon: '📊', color: '#f59e0b' },
              { label: 'Workspace',        value: stats.workspaces.toLocaleString('id-ID'),           icon: '🏢', color: '#8b5cf6' },
            ].map(card => (
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: '16px 20px', border: '1px solid #f1f5f9', marginBottom: 20 }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, maxWidth: 320 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: '#64748b' }}>Cari Workspace / Ternak / Batch</span>
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Nama workspace, ternak, atau tanggal…"
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f1f5f9', overflow: 'hidden', marginBottom: 32 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#0f172a' }}>Pemberian Pakan (Seluruh Platform)</span>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Workspace', 'Target', 'Tanggal', 'Waktu', 'Jumlah (kg)', 'Catatan', 'Dicatat'].map(h => (
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} style={{ padding: '48px 20px', textAlign: 'center', color: '#94a3b8' }}>
                      ...
                      <div style={{ fontWeight: 600, color: '#64748b' }}>
                        {rows.length === 0 ? 'Belum ada catatan pemberian pakan.' : 'Tidak ada hasil yang cocok'}
                      </div>
                    </td>
                  </tr>
                ) : pageRows.map((r, i) => (
                  <tr key={r.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafbfc', borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      {r.livestock ? (
                        <div>
                          <div style={{ fontSize: 12.5, color: '#0f172a' }}>{r.livestock.name ?? '—'}</div>
                          <div style={{ fontSize: 11, color: '#94a3b8' }}>{r.livestock.species ?? ''} · Individu</div>
                        </div>
                      ) : r.batches ? (
                        <div>
                          <div style={{ fontSize: 12.5, color: '#0f172a' }}>{r.batches.label ?? '—'}</div>
                          <div style={{ fontSize: 11, color: '#94a3b8' }}>Batch</div>
                        </div>
                      ) : '—'}
                    </td>
                    <td style={{ padding: '10px 14px', color: '#64748b', whiteSpace: 'nowrap', fontSize: 12 }}>{r.feed_date}</td>
                    <td style={{ padding: '10px 14px', color: '#64748b', fontSize: 12 }}>{r.feed_time ?? '—'}</td>
                    <td style={{ padding: '10px 14px', fontWeight: 700, color: '#0f172a', whiteSpace: 'nowrap' }}>
                      {r.amount_kg.toLocaleString('id-ID', { minimumFractionDigits: 1, maximumFractionDigits: 3 })} kg
                    </td>
                    <td style={{ padding: '10px 14px', color: '#64748b', fontSize: 12, maxWidth: 180 }}>{r.notes ?? '—'}</td>
