// ─── Admin Ownership Transfer — P0-005-018B ───────────────────────────────────
// Wired to adminOwnershipTransferData.ts (full list, live filter, real stats).

import { useMemo, useState } from 'react';
import AdminLayout from '../layout/AdminLayout';
import {
  ADMIN_TRANSFER_LIST,
  TRANSFER_PLATFORM_STATS,
  ...
  type TransferStatus,
} from '../../../data/adminOwnershipTransferData';
...
function StatusBadge({ status }: { status: TransferStatus }) {
  const c = TRANSFER_STATUS_CONFIG[status];
  if (!c) return <span style={{ fontSize: 11.5, color: '#64748b' }}>{status}</span>;
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px', borderRadius: 20, background: c.bg, color: c.color, fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap' }}>
...
function TransferDrawer({ record, onClose }: { record: OwnershipTransferRecord; onClose: () => void }) {
  const wt = WORKSPACE_TYPE_CONFIG[record.workspace.workspace_type] ?? { icon: '🏢', color: '#64748b', bg: '#f1f5f9' };
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 440, maxWidth: '100vw', background: '#fff', zIndex: 201, display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,0.12)' }}>
        <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: wt.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>{wt.icon}</div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', marginBottom: 2 }}>{record.workspace.workspace_name}</div>
            <div style={{ fontSize: 11.5, color: '#94a3b8', fontFamily: 'monospace', marginBottom: 6 }}>{record.request_id}</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 20px' }}>
          ...
          <InfoRow label="Workspace" value={record.workspace.workspace_name} />
          <InfoRow label="Tipe" value={record.workspace.workspace_type} />
          <InfoRow label="Lokasi" value={record.workspace.location} />
          <InfoRow label="Jumlah Ternak" value={record.workspace.livestock_count.toLocaleString('id-ID')} />
          <InfoRow label="Total Transaksi" value={record.workspace.total_transactions.toLocaleString('id-ID')} />
          <SectionLabel>Owner Saat Ini</SectionLabel>
          <InfoRow label="Nama" value={record.current_owner.full_name} />
          <InfoRow label="Email" value={record.current_owner.email} />
          <InfoRow label="Telepon" value={record.current_owner.phone ?? '—'} />
          <SectionLabel>Owner Baru (Diajukan)</SectionLabel>
          <InfoRow label="Nama" value={record.proposed_owner.full_name} />
          <InfoRow label="Email" value={record.proposed_owner.email} />
          <InfoRow label="Telepon" value={record.proposed_owner.phone ?? '—'} />
          <SectionLabel>Pengajuan</SectionLabel>
          <InfoRow label="Alasan" value={record.reason} />
          <InfoRow label="Diinisiasi Oleh" value={record.initiated_by} />
          <InfoRow label="Diajukan" value={formatDateShort(record.created_at)} />
          {record.acceptance_deadline && <InfoRow label="Batas Penerimaan" value={formatDateShort(record.acceptance_deadline)} />}
          {record.completed_at && <InfoRow label="Selesai Pada" value={formatDateShort(record.completed_at)} />}
          {record.notes && (
          ...
          {record.admin_notes && (
            <div style={{ marginTop: 8, padding: '10px 14px', borderRadius: 8, background: '#eff6ff', border: '1px solid #bfdbfe', fontSize: 12, color: '#1e40af' }}>🛡️ Admin: {record.admin_notes}</div>
          )}
        </div>
      </div>
    </>
...
export default function OwnershipTransferModule() {
  ...
  const filtered = useMemo(() => filterTransfers(ADMIN_TRANSFER_LIST, {
    search, status: filterStatus, workspaceType: '', dateFrom: '', dateTo: '',
  }), [search, filterStatus]);
  ...
  return (
    <AdminLayout>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: '#3b82f6', fontWeight: 600 }}>Transfer Kepemilikan</span>
          </div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: '#0f172a', letterSpacing: -0.3 }}>🔁 Transfer Kepemilikan</h1>
          <p style={{ margin: '6px 0 0', fontSize: 13.5, color: '#64748b' }}>
            Pemantauan transfer kepemilikan workspace — {ADMIN_TRANSFER_LIST.length} pengajuan ditampilkan.
          </p>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14, marginBottom: 24 }}>
          <StatCard label="Total Pengajuan" value={TRANSFER_PLATFORM_STATS.total} icon="🔁" color="#3b82f6" />
          <StatCard label="Dalam Proses"    value={TRANSFER_PLATFORM_STATS.pending} icon="⏳" color="#f59e0b" />
          <StatCard label="Selesai"         value={TRANSFER_PLATFORM_STATS.completed} icon="✅" color="#10b981" />
          <StatCard label="Dibatalkan"      value={TRANSFER_PLATFORM_STATS.cancelled} icon="❌" color="#ef4444" />
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: '16px 20px', border: '1px solid #f1f5f9', marginBottom: 20, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minWidth: 180 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: '#64748b' }}>Cari Transfer</span>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Workspace atau nama owner…"
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 160 }}>
            ...
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
              ...
              {(Object.keys(TRANSFER_STATUS_CONFIG) as TransferStatus[]).map(s => (
                <option key={s} value={s}>{TRANSFER_STATUS_CONFIG[s].label}</option>
              ))}
            </select>
          </label>
          {hasFilter && (
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #f1f5f9', overflow: 'hidden', marginBottom: 32 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: '#0f172a' }}>Daftar Transfer Kepemilikan</span>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Workspace', 'Tipe', 'Owner Lama', 'Owner Baru', 'Alasan', 'Status', 'Diajukan', 'Batas Penerimaan'].map(h => (
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  const wt = WORKSPACE_TYPE_CONFIG[r.workspace.workspace_type] ?? { icon: '🏢' };
                  return (
                    <tr key={r.request_id} onClick={() => setSelected(r)}
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        <div style={{ fontWeight: 600, color: '#0f172a', fontSize: 13 }}>{r.workspace.workspace_name}</div>
                        <div style={{ fontSize: 11, color: '#94a3b8', fontFamily: 'monospace' }}>{r.request_id}</div>
                      </td>
                      <td style={{ padding: '10px 14px', fontSize: 18 }}>{wt.icon}</td>
                      <td style={{ padding: '10px 14px' }}>
                        <div style={{ fontSize: 12.5, fontWeight: 500, color: '#0f172a' }}>{r.current_owner.full_name}</div>
                        <div style={{ fontSize: 11, color: '#94a3b8' }}>{r.current_owner.email}</div>
                      </td>
                      <td style={{ padding: '10px 14px' }}>
                        <div style={{ fontSize: 12.5, fontWeight: 500, color: '#0f172a' }}>{r.proposed_owner.full_name}</div>
                        <div style={{ fontSize: 11, color: '#94a3b8' }}>{r.proposed_owner.email}</div>
                      </td>
                      <td style={{ padding: '10px 14px', color: '#475569', fontSize: 12, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.reason}</td>
                      ...
                      <td style={{ padding: '10px 14px', color: '#64748b', whiteSpace: 'nowrap', fontSize: 12 }}>
                        {r.acceptance_deadline ? formatDateShort(r.acceptance_deadline) : '—'}
                      </td>
                    </tr>
                  );
                })}
      ...
      {selected && <TransferDrawer record={selected} onClose={() => setSelected(null)} />}
    </AdminLayout>
  );
}
